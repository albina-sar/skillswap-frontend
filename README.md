...
Разработкой переиспользуемых компонентов:

Input, Select, Popover, NotificationsContent

Реализацией страницы избранного:

Фильтрация навыков по лайкам из localStorage

Отображение карточек с помощью UserCard

Созданием логики уведомлений:

Подключение реальных уведомлений к UI (Header)

Добавление уведомления для отправителя при создании заявки

Кнопка «Перейти» → переход на страницу навыка

Настройкой UI Header и Footer:

Фиксация минимальной ширины, адаптив

Исправлением моковых данных:

Приведение городов в соответствие с данными пользователей

Устранение дублирования имени в уведомлениях

Работой над кнопкой лайка:

Подключение реальной логики на странице навыка (SkillLikeButton)

Вёрсткой и стилизацией компонентов с использованием CSS Modules и глобальных переменных
